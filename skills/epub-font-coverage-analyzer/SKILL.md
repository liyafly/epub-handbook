...
3. 查看：
   ...
   - 角色字符集：结合报告中的 text run、CSS 继承、覆盖和最终字体链核对，不只看普通 `p`。当前分析器不会采集 `quotes`、`content` 等 CSS 生成字符；须从 CSS 另行枚举，加入子集工具的 `extraCodepoints`，并对写出字体的 `cmap` 人工复核。
4. 按原因分派：
...
中文标点或数字样式异常时，先核对源码码位，再核对实际命中的字体角色及其 `cmap`，最后才判断阅读器渲染。`“”`（U+201C/U+201D）与 ASCII 引号、`〇`（U+3007）与 `○`（U+25CB）都不是可由子集化流程互换的字符。
