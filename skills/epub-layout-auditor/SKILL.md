...
1. 确定范围：
   ...
   - 若用户场景是「改造已有 EPUB」（而非「从零做新书」），本 skill 是入口；按 [SPEC §10](../../docs/final/SPEC-实现约束.md) 的红 / 黄 / 绿规则决定改动范围；每次改动跑 `scripts/validate_text_invariance.py` 红线 gate，红线触发立即回滚。人工 review 通过 `tools/epub-diff/index.html` 完成。
   - 典型脏 epub 模式对照 [docs/guides/epub-cleanup-patterns.md](../../docs/guides/epub-cleanup-patterns.md)。
