...
| `base.css` | `@page`、`html/body`、标题、段落、列表、表格、代码、基础 `figure/img`、内联语义、默认 Ruby |
| `notes.css` | 标准 footnote、popup note、多看 fallback 类 |
| `effects.css` | 着重号、波浪线 fallback、首字下沉、场景分隔 |
| `literary.css` | 章首、题记、对话、诗、信件、前置页 |
...
7. 保持 CSS 可读。某个层接近 200 行时，优先按职责拆分或移入已有正确层。
