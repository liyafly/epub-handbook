...
- 便签/资料卡保持真实文本；用 border/background/left-rule 做主路径，box-shadow/outline-offset 可丢失，通用 Kindle 版本不使用 `transform: rotate()`。
- 大合集需要总目录、局部目录和回目录链接分层；局部 `(•)` 回目录点可用，但要有可访问说明。
...
6. 处理装饰：
   ...
   - 便签、摘录卡和资料卡优先使用稳定边框；斜角感用不对称边框、圆角和投影模拟。
...
- 总目录放在 `nav.xhtml` / NCX，覆盖作品集或主要分卷。
- 每个作品集可有局部目录，锚点如 `id="collection-toc"`。
- 每篇短文末尾可以放“回本卷目录”链接；符号 `(•)` 可以作为视觉形式，但生产版需要 `title` / `aria-label` 或可见文案。
- 不用局部回目录点替代 OPF spine、nav 或 NCX。
- 合集过大时按作品集拆 XHTML，避免单文件过长导致编辑、转换和阅读器定位变慢。
...
- 不在通用 Kindle 版本用 `transform: rotate()` 旋转便签文本框；Kindle Previewer 3.104 实测会触发 KFX 转换内部错误。
