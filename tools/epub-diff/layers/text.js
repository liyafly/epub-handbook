...
async function textBlocks(epub, path) {
  const entry = epub.entryMap.get(path);
  if (!entry) return [];
  const doc = parseXml(await readEntryText(entry), path);
  const blocks = [];
...
export async function diffText(before, after) {
  ...
  for (const path of paths) {
    ...
    const bBlocks = await textBlocks(before, path);
    const aBlocks = await textBlocks(after, path);
