...
> 迁移状态：目标公开表面已确定为一个 Go CLI，详细设计见
> [`../docs/pipeline/go-cli-rearchitecture.md`](../docs/pipeline/go-cli-rearchitecture.md)。本页以下 Python/Swift
> 内容描述的是 Go 实现完成前的现状和差分基线；不得继续扩展 Swift，也不得在 parity gate 前删除旧入口。
...
`python/public-entrypoints.v1.json` 是所有 Python CLI / Agent 公开入口的 inventory；`python/provider-catalog.v1.json` 与 `scripts/python_provider_adapter.py` 是唯一允许执行 Python entrypoint 的 adapter 实现。它们只给迁移期 CLI / AI Agent 用，通过 request/result JSON 运行 allow-listed provider。冻结的 Swift package 与 GUI 不得调用它。
...
冻结的 Swift 基线 CLI 是 `swift run epub-handbook-swift`，仅暴露已完成双跑的
`inspect`、`validate-redlines` 与 `epub.notes.popup.normalize`。它不调用上面的
Python adapter，不再增加 capability，并将在 Go CLI 达到迁移蓝图的 parity/release gate 后与 GUI 一起删除。
