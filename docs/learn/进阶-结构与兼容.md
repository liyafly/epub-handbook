...
> 溯源：AGENTS.md §已有 EPUB 流程；`templates/epub-style-demo/META-INF/container.xml`。
...
> 溯源：AGENTS.md §已有 EPUB 流程；`epub run epub.package.nav.audit`。
...
不要长期维护一个“既不是严格 EPUB2，也不是完整 EPUB3”的混搭包。短期兼容实验可以做，但要在 reader matrix 中明确记录目标阅读器、版本、artifact、nav audit / redline 和 CI EPUBCheck 结果。
...
8. 运行 preflight、nav audit / redline、validator、CI EPUBCheck 和目标阅读器实测。
