...
**Q：`epub run epub.style.demo.maintain` 对我自己的 epub 报 fixture token？**
A：源树模式只服务本仓 demo（缺省 `--input` 时校验 `templates/epub-style-demo`）。
校验你自己的书用产物模式：`epub run epub.style.demo.maintain --input <你的.epub> --json`；
通用结构检查用 `epub run epub.package.nav.audit`，正文不变用 `epub redline`，
EPUBCheck 交给 GitHub Actions 跑。

**Q：CI 里的 EPUBCheck 报 fatal？**
A：先确认 zip 完整、`mimetype` 是第一个 entry、`META-INF/container.xml` 存在且能解析。本地先跑 `epub run epub.package.nav.audit --input <产物> --json` 定位包结构问题。

**Q：`epub redline` 报文本变化？**
...
**Q：skill 把我的正文文字改了？**
A：这是事故。回滚并运行 `epub redline --check text --verbose` 定位变化。
