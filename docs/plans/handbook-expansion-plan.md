...
>
> **落地状态（2026-05-27）**：Stage 1–4 的 v1 主路径已经落地；本文件保留为历史计划和后续审稿依据。当前状态以 `docs/pipeline/decisions.md`、`docs/plans/handbook-expansion-review.md` 和实际代码为准。
>
...
README     │  validate_text    │  Web App         │  city-field-   │
入门层 5 篇 │  cleanup-flow     │  index.html      │  notes garden  │
...
**v0.2 范围（Stage 4 之前补齐）**：扩到 metadata / spine / 封面 / DRM / 章节锚点。
...
**目的**：把 v0.2 范围（metadata / spine / cover / DRM / anchors 红线）**在 Stage 2 一次性做完**，否则「CI gate 守住所有红线」是不成立的承诺。原 S2-T2 写「v0.2 可延后到 Stage 4 前」，本任务把这部分提前。
...
- [x] `validate_text_invariance.py` v0.2 落地，覆盖文本 / metadata / spine / cover / DRM / anchors 红线。
- [x] `scripts/test_validate_text_invariance.py` 22 个测试用例通过。
...
- [ ] `bash tools/epub-diff/scripts/fetch-vendor.sh` 跑通，zip.js vendor 文件和 LICENSE 齐全。
...
> **2026-05-27 决策更新**：公版书 demo（鲁迅《呐喊》/《唐诗三百首》）永久搁置。Stage 4 唯一交付路径是 `samples/demo-books/` 自造样本。相关任务条目已从本文档移除。
...
> 原计划中的公版书 demo（鲁迅《呐喊》、《唐诗三百首》）已于 2026-05-27 决策永久搁置。Stage 4 唯一交付路径为自造样本。
...
[narrative：拿到了什么、用了哪些 skill、diff 工具里看到了什么]

详细记录见 `samples/demo-books/city-field-notes/notes.md`。
...
| Q8 | 是否补 `samples/third-party/` 第 3 本 | 已决策不做公版书 demo，搁置 |
...
> 2026-05-27 决策：首轮不做公版书 demo。以上链接保留作未来参考。
...
2. 按 §3.2 任务清单（扩展部分）：
   ...
   - S2-T9 `validate_text_invariance.py` v0.2（metadata / spine / cover / DRM / anchors 红线）
...
1. 按 §5.3 任务清单：
    ...
    - S4-T5 THIRD_PARTY.md 同步
2. commit 分两个：
    - `feat(samples): add self-built e2e cleanup demos`
    - `feat(docs): add getting-started case study`
3. 跑 §5.4 验收清单。

> 原计划的 S4-T1（鲁迅《呐喊》）和 S4-T2（《唐诗三百首》）已于 2026-05-27 决策永久搁置。
