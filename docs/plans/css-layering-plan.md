...
├── literary.css     — 文学结构 + 前置页（.dialog / .poetry / .letter / .scene-break / .chapter-head / .chapter-head-art / .chapter-head-banner / .epigraph / .copyright-page / .dedication / .epigraph-page / .english-fiction / .classical-modern）
...
| `literary.css` | 文学结构 + 前置页 | `.dialog` / `.poetry` / `.letter` / `.scene-break` / `.chapter-head` / `.chapter-head-art` / `.chapter-head-banner` / `.epigraph` / `.copyright-page` / `.dedication` / `.epigraph-page` / `.english-fiction` / `.classical-modern` / `.parallel-entry` | 弹注、图文浮动、海报、竖排 |
