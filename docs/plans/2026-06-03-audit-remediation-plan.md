...
- **做法**：`grep -rn '/Users/yafeili' docs/ scripts/ *.md` 找出**所有**出现处（审计在 `docs/plans/2026-05-28-readme-tools-followup-review.md:81` 和疑似 `docs/plans/2026-05-28-remove-epub-diff.md` 都见过），逐个泛化成相对路径或 `$PROJECT_ROOT`。
- ⚠️ 这些在历史计划里——只动「脚本片段里的路径」（卫生改进），不重写历史叙述。
- **验证**：改完 `grep -rn '/Users/yafeili' docs/ scripts/ *.md` 应为空（本计划文件本身含示例路径可豁免，或一并泛化）。
