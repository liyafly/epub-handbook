...
二进制内嵌 capability contracts、schemas 和样式预设，因此基本命令不依赖仓库 checkout。字体覆盖与字体子集化由独立可选 provider 提供，不包含在 CLI 归档里；具体安装与缺失行为见仓库 `tools-font/README.md`。

如需字体覆盖或书级构建中的字体子集 provider，请先获取此版本的仓库源码，再按 [`tools-font/README.md`](https://github.com/liyafly/epub-handbook/blob/v@VERSION@/tools-font/README.md) 安装。普通 EPUB 检查、结构处理和排版能力不需要安装 provider；调用依赖 provider 的字体能力时若缺少工具，CLI 会明确报告失败。
