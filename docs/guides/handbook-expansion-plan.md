...
> 3. **EPUB Diff 工具 v1 = 纯静态 Web App**（不是 AI 生成 HTML 报告）。用户「打开 → 选两个 epub → 看 diff」。继续用 [@pierre/diffs](https://www.npmjs.com/package/@pierre/diffs) 做核心 diff 渲染。**只做文件对比，不涉及阅读器效果验收**，不集成 Kindle Previewer 启动按钮。
> 4. **实测覆盖暂缓**。
...
- 核心 diff 渲染用 **[@pierre/diffs](https://www.npmjs.com/package/@pierre/diffs)**（与 YiTong 同源）。
- 浏览器内解压用 **[zip.js](https://gildas-lormeau.github.io/zip.js/)**（`@zip.js/zip.js`，30KB，流式 + 随机访问，**不一次性把整包加载进内存**）。
- 配合 `Blob.stream()` 和 `crypto.subtle.digest`，资源层 SHA-256 边流边算，字节不驻留。
- 设计目标支撑到 **~1.5GB epub**（普通桌面 / 笔记本 / 现代移动浏览器都能跑）。
- **只做文件对比**，不涉及任何阅读器效果验收（不嵌阅读器 / 不渲染 epub / 不启动 Kindle Previewer）。
- 离线可用：所有依赖（pierre-diffs、zip.js）本地化在 `assets/vendor/` 下。
...
- **Web app 前端**（Stage 3 用）：本地化的 zip.js + @pierre/diffs，**无 Node 运行时**。
...
pipeline 占位│ samples 占位      │  zip.js + pierre │  case study   │
            │                  │                  │                │
≈ 2 周      │  ≈ 3 周           │  ≈ 3 周           │  ≈ 1.5 周        │
```
...
4. [04-skills.md](04-skills.md) — AI skills 是什么、怎么用
5. [05-case-study.md](05-case-study.md) — 自造 EPUB 清洗演示案例
...
| 步骤 | 工具 | 文档 |
| --- | --- | --- |
| intake | `scripts/epub_ai_harness.py <epub>` | [epub-cleanup-flow.md](../guides/epub-cleanup-flow.md) |
...
- [guides/](guides/)
  ...
  - 清洗流程：[epub-cleanup-flow.md](guides/epub-cleanup-flow.md)（Stage 2 落地）
  - 典型脏 epub 模式：[epub-cleanup-patterns.md](guides/epub-cleanup-patterns.md)（Stage 2 落地）
  - 资源优化（图片 / 字体）：[asset-optimization.md](guides/asset-optimization.md)（Stage 2 落地）
  - Diff 工具：[epub-diff-tool.md](guides/epub-diff-tool.md)（Stage 3 落地）
...
跑通 [01-first-epub.md](01-first-epub.md) 之后，你应该能把自己的 epub 跑一遍这套工具链。
...
把 §4 的 findings 对照 [docs/guides/epub-cleanup-flow.md](../guides/epub-cleanup-flow.md)：
...
- 看 [glossary.md](glossary.md) 确认术语理解一致。
...
- **epubcheck**：W3C 官方 epub 校验器；行业 baseline 工具。详见 [01-first-epub.md](01-first-epub.md)。
...
- **SubtleCrypto**：浏览器 Web Crypto API；本仓 web app 用它做 SHA-256。
...
1. [01-first-epub.md](01-first-epub.md) — 5 分钟跑通示例
2. [02-anatomy.md](02-anatomy.md) — 理解 epub 是什么
3. [03-readers.md](03-readers.md) — 知道要测哪些 reader
4. [06-test-your-own.md](06-test-your-own.md) — 测自己的 epub
5. [04-skills.md](04-skills.md) — 了解 AI 协作能力
6. [07-faq.md](07-faq.md) — 卡住了再翻
7. [05-case-study.md](05-case-study.md) — 看自造 EPUB 清洗演示案例
8. [glossary.md](glossary.md) — 随时查术语
````
...
#### S2-T3：新增 `docs/guides/epub-cleanup-flow.md`
...
工具的详细使用见 [epub-diff-tool.md](epub-diff-tool.md)。
...
#### S2-T8：新建 `docs/guides/epub-cleanup-patterns.md`「典型脏 epub 模式目录」

**目的**：把「拿到一本 epub，怎么识别是哪种问题模式 + 推荐 skill 顺序」变成可查表的指南，是 `epub-layout-auditor` 决策的具体细化。

**输入**：无。
**输出**：`docs/guides/epub-cleanup-patterns.md`。
**时间估算**：4–5 小时（核心是模式整理）。
...
```sh
test -f docs/guides/epub-cleanup-patterns.md
grep -cE "^## 模式 [A-Z]：" docs/guides/epub-cleanup-patterns.md   # 期望 ≥ 7 个模式
```
...
#### S2-T10：扩展 `docs/guides/epub-cleanup-flow.md`，加 7 个新章节
...
匹配模式（参 [epub-cleanup-patterns.md](../../docs/guides/epub-cleanup-patterns.md)）：模式 B。
...
```sh
grep -q "## 9. 批量模式" docs/guides/epub-cleanup-flow.md
grep -q "## 10. 回滚剧本" docs/guides/epub-cleanup-flow.md
grep -q "## 11. 可信度评估" docs/guides/epub-cleanup-flow.md
grep -q "## 12. 错误恢复" docs/guides/epub-cleanup-flow.md
grep -q "## 13. OCR-style 脏 epub 识别" docs/guides/epub-cleanup-flow.md
grep -q "## 14. 标准" docs/guides/epub-cleanup-flow.md
grep -q "epubcheck" docs/guides/epub-cleanup-flow.md
```
...
dry-run 输出格式见 [docs/guides/epub-cleanup-flow.md §11](../../docs/guides/epub-cleanup-flow.md)。
````
...
按 [epub-cleanup-patterns.md](../guides/epub-cleanup-patterns.md) 的「模式 B（出版社旧版）」举例：
...
参 [epub-cleanup-flow.md](epub-cleanup-flow.md)：
...
- [epub-cleanup-flow.md](epub-cleanup-flow.md) — 本工具调用所在的清洗流水线
- [epub-cleanup-patterns.md](epub-cleanup-patterns.md) — 典型脏 epub 模式，本指南是其中「资源层」处理的展开
...
```sh
test -f docs/guides/asset-optimization.md
grep -q "oxipng" docs/guides/asset-optimization.md
grep -q "pyftsubset" docs/guides/asset-optimization.md
grep -q "cover-image" docs/guides/asset-optimization.md
grep -cE "^## " docs/guides/asset-optimization.md   # 期望 ≥ 8 个一级章节
markdownlint-cli2 docs/guides/asset-optimization.md
```
...
- [ ] `docs/guides/epub-cleanup-flow.md` 完整：§1 健康检查（含 epubcheck）+ §2–§8 主流程 + §9 批量 + §10 回滚 + §11 评估 + §12 恢复 + §13 OCR 识别 + §14 notes 模板。
- [ ] `docs/guides/epub-cleanup-patterns.md` 落地，覆盖 ≥ 7 个典型模式。
...
- [ ] `docs/guides/asset-optimization.md` 落地，覆盖图片转换 / 无损压缩 / 字体子集化；命令全用现有工具（oxipng / mozjpeg / webp / pyftsubset / glyphhanger）。
...
- **纯前端**（浏览器内运行）：zip.js 流式解压 + JavaScript 解析 + @pierre/diffs 渲染。
...
⊞ Text ────────────────────────────────────────────────────────────────
   ...
   │           inline diff rendered by @pierre/diffs                │
   └────────────────────────────────────────────────────────────────┘
...
@pierre/diffs 自身用 Shiki 主题（`github-light` / `github-dark`），跟随 viewer 主题。

#### 4.2.10 移动端折叠（< 768px）

- 侧栏 Layers 折成顶部水平 chip 栏。
- 五层 section 默认全部折叠。
- @pierre/diffs 强制 Stacked 视图（不水平滚动）。
...
│  License: MIT                                    │
│  Diff rendering: @pierre/diffs (MIT)             │
...
[ 五层 diff（按需流式读取每个 entry） ]
   ...
   ├─ resources.js: 资源 entry → WritableStream → crypto.subtle.digest 流式 hash
...
[ 构造 diff 数据对象（轻量 JSON，几 MB 级）]
        │
        ▼
[ render.js + <pierre-diff> Web Components ]
        │
        ▼
[ DOM 渲染 ]
```
...
- **Hash 边流边算**：资源层用 `crypto.subtle.digest('SHA-256', ...)` 接 ReadableStream，字节不进 JS heap。
...
- 文件路径 + 每段 hash + 状态。
- 段落变化时，用 @pierre/diffs 渲染该段的 inline diff。
...
3. **同时**保留原始文本，喂给 `<pierre-diff>` 做 line-level diff。

输出：

- selector 添加 / 删除 / 修改 count。
- 每个 CSS 文件的完整 line diff（pierre-diffs 渲染）。
- @media 查询变化。
...
1. 按 path 配对。
2. **流式 sha256**：用 zip.js 的 `entry.getData()` 接到一个自定义 `WritableStream`，把每个 chunk 喂给增量 SHA-256；不在 JS heap 中保留整个字节流。
   - 增量 hash 用 [hash-wasm](https://github.com/Daninet/hash-wasm)（~50KB WASM，stream API 友好）。
   - 退化方案：单 entry 一次性读取后 `crypto.subtle.digest`，单 entry < 50MB 时可接受。
...
# @pierre/diffs
npm pack "@pierre/diffs@$PIERRE_VERSION"
tar -xzf pierre-diffs-*.tgz
cp package/dist/diffs.js   "$ASSETS/pierre-diffs.js"
cp package/dist/diffs.css  "$ASSETS/pierre-diffs.css"
cp package/LICENSE         "$ASSETS/pierre-diffs.LICENSE"

# zip.js (@zip.js/zip.js, BSD-3-Clause)
# 用 dist/zip-full.min.js（含解压 + Web Worker 支持）
npm pack "@zip.js/zip.js@$ZIPJS_VERSION"
tar -xzf zip.js-zip.js-*.tgz 2>/dev/null || tar -xzf zip.js-*.tgz
cp package/dist/zip-full.min.js "$ASSETS/zip.js"
...
zip.js 版本选 `dist/zip-full.min.js`（含 inflate + Web Worker 支持），既能流式解压又能在 Worker 里异步跑（v0.2 优化预留）。
...
<head>
  ...
  <link rel="stylesheet" href="assets/vendor/pierre-diffs.css">
...
<body>
  ...
  <!-- zip.js 是 ES module；app.js 也用 type="module" 引用 -->
  <script type="module">
    // 把 zip.js 暴露到 window 给 app.js 用（或在 app.js 里 import）
    import * as zip from './assets/vendor/zip.js';
    window.zip = zip;
  </script>
  <script src="assets/vendor/pierre-diffs.js"></script>
...
│   ├── loading.js
│   ├── diff-view.js
│   ├── pierre-diff.js           # 包装 <pierre-diff> Web Component 调用
│   └── theme.js
└── util/
    ├── hash.js                  # SubtleCrypto SHA-256
    └── format.js                # 字节数 → 人类可读
```
...
The tool uses **zip.js** with streaming reads + `Blob.stream()`, so the
entire EPUB is **never loaded into memory**. Only the central directory
(a few KB) is read first, and individual entries are decompressed on
demand. Peak memory ≈ size of the largest single entry + diff state,
usually under 50 MB regardless of EPUB size.

Tested working range: up to ~1.5 GB EPUBs on a typical laptop browser.
...
- **Style**: CSS selectors and properties (rendered by @pierre/diffs).
...
- [@pierre/diffs](https://www.npmjs.com/package/@pierre/diffs) (license per upstream) — diff renderer
...
| WT3 | demo epub 改 CSS（如 T1 双 float 改动）| Style 层显示 pierre diff |
...
test -f tools/epub-diff/assets/vendor/pierre-diffs.js
...
- HTML 转义：把 CSS 内容塞进 `<pierre-diff>` 属性时要正确 escape。
...
- `fetch-vendor.sh` 顶部明确写 `PIERRE_VERSION` 和 `JSZIP_VERSION` 变量。
- 不用 `latest`；落地时填具体版本号。
- 升级版本需要明确 commit `chore(vendor): bump pierre-diffs to X.Y.Z`，且 README 更新。
...
# 期望看到 4 个文件：pierre-diffs.js/css、zip.js、对应 LICENSE
```

---

#### S3-T3：`docs/guides/epub-diff-tool.md`
...
见 [epub-cleanup-flow.md](epub-cleanup-flow.md)。
...
| CSS diff 看不到颜色 | pierre-diffs.css 没加载；检查 vendor 目录 |
````

**验收**：

```sh
test -f docs/guides/epub-diff-tool.md
grep -q "tools/epub-diff/index.html" docs/guides/epub-diff-tool.md
grep -q "fetch-vendor.sh" docs/guides/epub-diff-tool.md
```
...
- [ ] `docs/guides/epub-diff-tool.md` 落地，链接正确。
...
> 日期：<DATE>
> 操作者：<NAME>
> Source：见 [metadata.yaml](metadata.yaml)
> 流程：见 [docs/guides/epub-cleanup-flow.md](../../../docs/guides/epub-cleanup-flow.md)
...
详细记录见 [samples/third-party/lu-xun-nahan/notes.md](../../samples/third-party/lu-xun-nahan/notes.md)。
...
详细记录见 [samples/third-party/tang-poems-300/notes.md](../../samples/third-party/tang-poems-300/notes.md)。
...
- 拿你自己的 epub，跟着 [docs/guides/epub-cleanup-flow.md](../guides/epub-cleanup-flow.md) 跑一遍。
...
- 许可：见 `tools/epub-diff/assets/vendor/pierre-diffs.LICENSE`（运行时本地化）
...
**不引入**：tinycss2、Jinja2、BeautifulSoup、PyYAML、Pandas、Pillow、任何 Node.js runtime（@pierre/diffs 和 zip.js 都是浏览器端 ES module）、任何 ML/NLP 库、任何 Web 框架、Electron、JSZip（被 zip.js 取代）。
...
| Q2 | Vendor 资源（pierre-diffs, zip.js）是否入 git | 不入（生成式） | 体积 + license 文件单独入仓即可 |
...
2. **@pierre/diffs** — Web Component / JS diff 渲染库
   <https://www.npmjs.com/package/@pierre/diffs>
   <https://diffs.com/>

3. **opencode-diffs** — 用 @pierre/diffs 的 web 端实现（参考）
