...
> **审稿对象**：`docs/guides/handbook-expansion-plan.md`（Claude 草稿，2026-05-26）
...
1. `docs/guides/epub-cleanup-flow.md` 文档结构破损（§15 错位到 §14 模板代码块中）。
...
| S2-T3 epub-cleanup-flow.md 骨架 | ✅ 但 §14/§15 结构破损（P0，见 2.2） |
...
| S2-T8 epub-cleanup-patterns.md | ✅ 8 个模式（A..H） |
...
**P0：`docs/guides/epub-cleanup-flow.md` §14/§15 结构破损**
...
1. **fix(guides): 修 epub-cleanup-flow.md §14/§15 结构** — 把 §15 提出 §14 的 notes.md 模板代码块；不动 §14 模板原意。
...
| S3-T3 epub-diff-tool.md | ✅ 85 行；含安装、demo 文件指引、`file://` 解决方案 |
| S3-T4 根 README 入口 | ✅ |
...
如果暂不想引入 `hash-wasm`，把 README + epub-diff-tool.md + handbook-expansion-plan.md 全部改成：
...
- README / epub-diff-tool.md / handbook-expansion-plan.md §0.1 / §4.2.5 / §4.4.3 / §4.5 全部把 @pierre/diffs 替换成「内置 line diff renderer」。
...
>
> - M1.0 Stage 1：✅ 完成
> - M1.1 Stage 2：⚠ 完成主线，遗留两处文档/口径偏差，详见 docs/guides/handbook-expansion-review.md
> - M1.2 Stage 3：⚠ 主体完成，流式 hash 和 @pierre/diffs 集成未落地承诺，详见 review
> - M1.3 Stage 4：✅ 用自造样本完成；公版书 demo 永久搁置
> - 续做项的优先级与具体修法见 docs/guides/handbook-expansion-review.md
```
...
| --- | --- | --- | --- | --- |
| 1 | P0 | fix | `fix(guides): 修复 epub-cleanup-flow.md §14/§15 结构破损` | `docs/guides/epub-cleanup-flow.md` |
| 2 | P0 | feat / docs | 二选一：`feat(tools): 用 hash-wasm 实现流式 SHA-256` OR `docs(plan,tools): 把流式 hash 承诺降级为 v0.2 follow-up` | `tools/epub-diff/util/hash.js`、`parsers/epub.js`、`layers/resources.js`、`README.md`、`docs/guides/epub-diff-tool.md`、`docs/guides/handbook-expansion-plan.md` |
...
| 9 | P2 | docs | `docs(plan): 在 handbook-expansion-plan.md 顶部加落地状态摘要 + §5 公版书决策 banner` | `docs/guides/handbook-expansion-plan.md` |
...
1. **`docs/guides/` 身份混杂**：16 个文件里包含
   ...
   - **(c) 清洗流水线流程文档**（应迁到 pipeline/）：`epub-cleanup-flow.md`、`epub-cleanup-patterns.md`、`asset-optimization.md`
   - **(d) 工具说明**（应迁到 pipeline/ 或就近）：`epub-diff-tool.md`
...
│   ├── decisions.md                          # 不动
│   ├── skills-matrix.md                      # 不动
│   ├── cleanup-flow.md                       # ← 从 guides/epub-cleanup-flow.md 迁
│   ├── cleanup-patterns.md                   # ← 从 guides/epub-cleanup-patterns.md 迁
│   ├── asset-optimization.md                 # ← 从 guides/ 迁
│   └── diff-tool.md                          # ← 从 guides/epub-diff-tool.md 迁
│
...
```sh
git mv docs/guides/handbook-expansion-plan.md     docs/plans/
git mv docs/guides/handbook-expansion-review.md   docs/plans/
git mv docs/guides/css-layering-plan.md           docs/plans/
git mv docs/guides/fonts-css-expansion-plan.md    docs/plans/
git mv docs/guides/demo-scene-expansion-plan.md   docs/plans/
git mv docs/guides/skills-and-templates.md        docs/plans/
```
...
- 任何 `skills/*/SKILL.md` 内对这些 plan 文档的引用（grep `guides/handbook-expansion-plan\|guides/css-layering\|guides/fonts-css-expansion\|guides/demo-scene-expansion\|guides/skills-and-templates`）
...
```sh
git mv docs/guides/epub-cleanup-flow.md      docs/pipeline/cleanup-flow.md
git mv docs/guides/epub-cleanup-patterns.md  docs/pipeline/cleanup-patterns.md
git mv docs/guides/asset-optimization.md     docs/pipeline/asset-optimization.md
git mv docs/guides/epub-diff-tool.md         docs/pipeline/diff-tool.md
```

**文件名变化**（去掉 `epub-` 前缀，因为 pipeline/ 上下文已经隐含 epub）：

- `epub-cleanup-flow.md` → `cleanup-flow.md`
- `epub-cleanup-patterns.md` → `cleanup-patterns.md`
- `epub-diff-tool.md` → `diff-tool.md`
- `asset-optimization.md` 保持原名（不带 epub- 前缀）。

迁完后必须更新引用（grep 这些旧路径）：

- `docs/final/SPEC-实现约束.md` §10.5 中的 `tools/epub-diff/` 引用不变；但其他地方如果引用了 guides/epub-cleanup-flow 要改。
- `docs/getting-started/04-skills.md`、`05-case-study.md`、`06-test-your-own.md` 都引用了 guides/epub-cleanup-flow.md。
- `docs/pipeline/README.md`、`docs/pipeline/skills-matrix.md`、`docs/pipeline/decisions.md` 互相引用。
- `tools/epub-diff/README.md` 引用 `docs/guides/epub-diff-tool.md`。
...
- `CLAUDE.md` 优先级表里如果有具体文件路径，按新位置更新（grep `docs/guides/handbook\|docs/guides/css-layering\|docs/guides/fonts-css-expansion\|docs/guides/demo-scene-expansion\|docs/guides/skills-and-templates\|docs/guides/epub-cleanup\|docs/guides/epub-diff\|docs/guides/asset-optimization\|docs/final/epub-pro\|docs/final/fixtures.md\|docs/reference\|docs/experiments/EPUB 3 章节扉页`）。
...
# 5. 旧路径全无遗留
! grep -rn "docs/guides/handbook-expansion-plan\|docs/guides/css-layering\|docs/guides/fonts-css-expansion\|docs/guides/demo-scene-expansion\|docs/guides/skills-and-templates" . --include="*.md" --include="*.yaml" --include="*.py" --include="*.sh"
! grep -rn "docs/guides/epub-cleanup\|docs/guides/epub-diff\|docs/guides/asset-optimization" . --include="*.md" --include="*.yaml" --include="*.py" --include="*.sh"
...
- §6 #1（修复 epub-cleanup-flow.md §14/§15）涉及 `docs/guides/epub-cleanup-flow.md`；如果先迁到 `docs/pipeline/cleanup-flow.md` 再修，git 历史更干净。
- §6 #2 / #3（流式 hash + pierre-diffs）涉及 `tools/epub-diff/README.md` 与 `docs/guides/epub-diff-tool.md`；如果先迁到 `docs/pipeline/diff-tool.md` 再改，可避免一次冲突。
...
>
> §9 的目录重组在执行后会让本文件自身从 `docs/guides/handbook-expansion-review.md` 移动到 `docs/plans/handbook-expansion-review.md`。届时所有外部对本文件的引用都需要同步更新（README、CLAUDE.md、PR 描述等）。
