...
| `effects.css` | `.emp` / `.wavy` / `.dropcap` / `.dropcap-host` / `.scene-break` / `.scene-break-text` / `.has-ruby`（Ruby 行距加强） | `.kaiti` 的字体声明（那是 fonts.css 的事；这里只能加排版微调如 `.kaiti { text-indent: 0; }` 这种） |
