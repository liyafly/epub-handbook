...
| `effects.css` | `.emp` / `.wavy` / `.dropcap` / `.dropcap-host` / `.scene-break` / `.scene-break-text` / `.has-ruby`（Ruby 行距加强）/ `.note-box` 边框阴影类（含专业花边框、长条投影） | `.kaiti` 的字体声明（那是 fonts.css 的事；这里只能加排版微调如 `.kaiti { text-indent: 0; }` 这种） |
...
| `effects.css` | 文字效果 + 便签视觉 | `.emp` / `.wavy` / `.dropcap` / `.scene-break` / Ruby 行距 / `.note-box` 边框阴影类（含专业花边框、长条投影） | 字体声明、弹注、文学结构 |
