...
- `cleanup-flow.md`：已有 EPUB 清洗流水线，覆盖健康检查、红线 gate、diff review、批量、回滚和 notes 模板。
- `cleanup-patterns.md`：典型脏 EPUB 模式与推荐 skill 顺序。
- `asset-optimization.md`：图片与字体优化指南，覆盖 WebP 转换、无损压缩、字体子集化和封面红线。
- `diff-tool.md`：纯静态 EPUB Diff Web App 的使用说明。
