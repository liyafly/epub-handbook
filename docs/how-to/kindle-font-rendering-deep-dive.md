...
正文角色使用一款覆盖其全部实际字符的字体，可减少 Kindle 触发不可靠回退。它既可用于生僻字很多的书，也可用于设计上必须固定正文外观的锁定版；需配合 `fontspec=forceAll`、直接 `body` 规则和 OPF `ibooks:specified-fonts=true`。
...
### 3.1.1 中文标点与相似字符：先分码位、覆盖和字形

中文引号、波浪号、圆圈和年份中的零“看起来不对”时，按三层检查，不先改正文：

1. **源文字码位**：确认 XHTML 实际保存的字符。例如中文双引号通常是 `“` U+201C / `”` U+201D，不是 ASCII `"` U+0022；年份中的 `〇` U+3007 也不是几何圆 `○` U+25CB。
2. **字体覆盖与实际角色**：确认该字符最终继承哪个 `font-family`，再检查原字体与子集字体的 `cmap`。缺少码位时看到的可能是系统 fallback，不是目标字体自身字形。
3. **阅读器显示**：在相同字号和设置下比较原字体、子集字体与正文自由模式。相同码位在不同字体里可以有不同造型；视觉不同不能反推正文被替换。

子集化、重命名或把扩展名改成小写都不应改变 XHTML 码位。`〇` / `○`、全角 / 半角标点或不同波浪号之间的替换属于编辑决策，必须通过正文 diff 明确授权。若 `quotes`、`content` 生成引号等字符，这些字符虽不直接出现在 XHTML，也要另行枚举：C1-body 必须加入正文角色子集；只有局部角色才可改走已明确并验证的 fallback。
