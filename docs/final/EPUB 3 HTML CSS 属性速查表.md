...
| 文白对照条目 | `<section>` + 真实标题 + 原文/白话段落 + 宽屏 float 对照 | 推荐 | 默认源序上下；仅 `min-width` 宽阅读区域左右显示；不用 `table` / `flex` / `grid` 承载正文 |
...
| `float` | `left` / `right` | 推荐 | 图文环绕通用路径：float 挂 `<figure>`，宽度用 `25%–35%` 百分比；文白对照只在 `min-width` 宽屏增强中用双段落 `float:left/right` + `38%/58%`；英文首字主路径用 `::first-letter`。 |
| 文白对照浮动列 | `@media (min-width:40em)` 内 `.parallel-float-pair .classical-text { float:left; width:38% }` + `.modern-text { float:right; width:58% }` + `.parallel-clear` | 条件可用 | 默认上下；短组宽屏才左右；长段用 `.parallel-stack-pair` 上下并允许分页 |
...
| `parallel text + wide float enhancement` | 可用 | 可用 | 可用 | 条件可用 | 可用 | 推荐；Kindle 窄屏默认上下，宽屏才试 38/58 左右，失败或长段保持源序上下 |
