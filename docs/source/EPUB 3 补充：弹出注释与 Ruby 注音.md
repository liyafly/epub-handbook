...
> 国内主流（多看 5.x+、掌阅 2020+、微信读书、京东读书）兼容方案见 §一.6。
>
> 系列其他文档：
...
CSS 沿用 §6.2 不改（多看 / 掌阅在自有阅读器内强制套用自己的弹层样式，作者 CSS 多被忽略）。
...
| 多看 4.x 及更早 | 需独立 `<ol class="duokan-footnote-content">` 列表 | 回退跳转（结构限制，不覆盖） |
| 掌阅（2020 后版本） | 原生识别 `epub:type="noteref"` + `<aside>` | 弹出，无需 §6.6 改动 |
| 掌阅（老版本） | 需 `<img zy-footnote="...">` 注释图标 | 回退跳转（结构冲突，不覆盖） |
| 微信读书 | 仅识别 EPUB 3 标准 | §6.1 即支持 |
| 京东读书 / 网易蜗牛 | 识别 EPUB 3 标准 | §6.1 即支持 |

**接受的妥协**：要让多看 4.x / 掌阅老版完全弹出，必须额外写一份独立的 `<ol class="duokan-footnote-content">` 或 `<img zy-footnote>` 结构——这会破坏"单一结构"原则，本手册不覆盖。在那两个老版本上，§6.6 会退化为页面跳转，**仍可读，只是不弹**。
