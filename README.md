...
3. **AI 协作 skills** — [skills/](skills/)：2 个主入口（`epub-layout-auditor` / `epub-source-intake`）+ 17 个专项 skill。可被 Claude Code / Codex 直接调用，也可由人工照 `SKILL.md` 步骤执行。
...
| 判断文本结构角色与字体/排版策略 | `python3 scripts/epub_content_analyzer.py <input> --format json` + `$epub-content-analyzer` |
| 检查生僻字、缺字和字体回退 | `python3 scripts/epub_font_coverage_adapter.py <input.epub> --format json` + `$epub-font-coverage-analyzer` |
| 合并 / 拆分 EPUB，或编辑封面 / 元数据 | [docs/pipeline/package-operations.md](docs/pipeline/package-operations.md) + `$epub-package-operator` |
...
| 合并、拆分、封面、元数据 | `python3 scripts/epub_package_merge_harness.py --help` 等单能力入口 | 完整映射见 [docs/pipeline/package-operations.md](docs/pipeline/package-operations.md)；旧 `epub_package_tool.py` 保持兼容 |
| 规划 / 执行 EPUB3 迁移 | `python3 scripts/epub3_migration_harness.py <input.epub>` / `python3 scripts/epub3_migration_apply_harness.py <input.epub> --output <out.epub>` | `$epub3-migrator` 规定 preflight、plan、apply 和红线顺序 |
...
| 分析文本结构角色 | `python3 scripts/epub_content_analyzer.py <input> --format json` | 支持 EPUB/XHTML/HTML/Markdown/TXT；默认不输出正文片段 |
| 分析字体覆盖 | `python3 scripts/epub_font_coverage_adapter.py <input.epub> --format json` | 通过独立 uv 项目检查 cmap、缺字和 reader profile 风险 |
...
| 校验 AI 入口、skills 和文档一致性 | `python3 scripts/validate_ai_entrypoints.py` / `python3 scripts/validate_skills_basic.py` / `python3 scripts/validate_docs_consistency.py` | 改维护文档、skill 或字体规则后运行 |
...
| `scripts/epub3_migration_apply_harness.py` | 显式写出 EPUB3 迁移产物，不覆盖已有输出 | capability、before/after SHA-256、conversion report |
...
全部 19 个 skill（2 个主入口 + 17 个专项）见 [docs/learn/04-skills.md](docs/learn/04-skills.md) 反向查表。
