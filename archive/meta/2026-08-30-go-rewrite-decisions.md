...
>
> 规范来源是 [Go 架构 SPEC](../../docs/final/SPEC-go-architecture.md)（第一档硬约束）。
...
| CLI ready 能力 | 17 / 22（其余 5 个 C 类纯 AI skill 设计上无 Go 实现；`epub.source.intake` 已于 2026-09-07 以最小盘点 planner 落地） |
...
**C 类（5 个，纯 AI skill，无专属实现，按 §7.1 不建 caps 包）**：
...
**关键语义决策（迁移期确定）**：
1. `epub run <id>` 执行 `ResolveChain` 得到的**完整 requires 链**（上游在前、目标在后，
   2026-09-01 起）。上游 stage 是**非阻断诊断**：其 Status 与完整 findings 落入
   `facts["<id>.status"]` / `facts["<id>.findings"]`，信封 `findings` 只追加一条
   `info upstream.diagnostics` 摘要（N+M>0 时），事件为 `<id>:completed`
   （message `diagnostic: N error, M warn`）；上游 Status failed 或 error finding
   **不改变**目标 stage 的执行与信封状态（真书上 nav.audit 几乎必然报 error）。
   仍阻断的只有：DRM preflight、runner 返回 Go error（`capability.run-failed`，
   工具坏了 ≠ 书有问题）、链上任一能力无 Go 实现（`capability.not-implemented`）。
   上游结果同时经 `Upstream` 注入目标 runner；目标 stage 的 findings 照常进信封，
   其 Status failed / error finding → 信封 failed、退出码 1、不落盘。
2. 红线在内存态（`OriginalState` vs `CurrentState`，链上 redLines 并集）校验，
   输出仍**只写一次**（INV-3）：红线 error / 校验器错误把状态降为 failed、退出码 1，
   但**不阻止落盘**，信封 `output.path` / `output.sha256` 照常给出，供人工 diff review
   （对齐 Python「写 after → gate → 人工 review」；metadata.edit / merge /
   cover.replace / migrate.epub3 的契约红线本就会被自身的预期变更触发）。
   落盘只被 DRM preflight、runner 错误、未实现能力、目标 stage 失败与 dry-run 阻断。
...
4. 全局 flag（input/output/dry_run；`legacy_report` 已于 2026-09-04 移除）由 pipeline 注入 Args。
...
（该脚手架已于 2026-09-04 整体拆除，CLI flag / 参数 / `facts.legacyReport` 均不复存在，
其数据已提升为正式 `facts` 键；见 §6 遗留项 2）。
