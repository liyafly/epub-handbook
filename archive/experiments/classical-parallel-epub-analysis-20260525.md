...
- `docs/how-to/anthology-navigation.md`：补充超大古籍/工具书的条目级目录规则。
- `原 fonts-css-expansion-plan（2026-06-25 仓库整合中移除，内容已并入 SPEC §8）`：补充 active CSS `url()` / `@font-face src` 必须同时存在于 zip 与 OPF manifest 的检查项。
...
- `docs/how-to/classical-modern-layout.md`：把“不要双栏”修正为“不要 table/flex/grid/固定版式；允许 float 渐进增强”。
