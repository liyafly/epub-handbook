...
from .detectors import DETECTORS, Detector, collect_actionable_findings, detector
from .model import EpubModel, build_model, build_model_from_tree
from .report import LEVEL_ORDER, Report, render_markdown
...
def inspect_opf(
  ...
  if package_version and not package_version.startswith("3"):
    ...
    report.add_skill("epub3-migrator", "warn")
    report.add_skill("epub-package-nav-auditor", "warn")
    if report.input_kind == "existing-epub":
      report.add_command(
        f"python3 scripts/epub3_migration_harness.py {shell_path(Path(report.input))} --format json"
      )
      report.add_command(
        f"python3 scripts/epub3_migration_apply_harness.py {shell_path(Path(report.input))} "
  ...
  if media_counts["xhtml"]:
    report.add_skill("epub-content-analyzer")
    if report.input_kind == "existing-epub":
      report.add_command(f"python3 scripts/epub_content_analyzer.py {shell_path(Path(report.input))} --format json")
  if media_counts["images"]:
    report.add_skill("epub-image-layout-optimizer")
  if media_counts["fonts"]:
    report.add_skill("epub-font-coverage-analyzer")
    if report.input_kind == "existing-epub":
      report.add_command(f"python3 scripts/epub_font_coverage_adapter.py {shell_path(Path(report.input))} --format json")
...
def inspect_epub_tree(path: Path, report: Report, opf_path: Path) -> None:
  ...
  report.model = build_model_from_tree(root_dir, rel_opf, opf_root)
...
def inspect_source(path: Path, report: Report) -> None:
  ...
  if suffix in PDF_EXTS:
    ...
    report.add_skill("epub-content-analyzer")
    report.add_command(f"python3 scripts/epub_content_analyzer.py {shell_path(path)} --format json")
...
def main(argv: list[str]) -> int:
  from .routing import inspect_path as route_input
  ...
  report = route_input(args.path, args.mode)
  data = report.as_dict()
  if args.format == "json":
