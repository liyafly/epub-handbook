...
def validate_source(check: Check) -> None:
  ...
  check.require("page-break-inside: avoid" in pair_block, "parallel-pair must declare page-break-inside: avoid for Kindle-safe pagination")
  check.require("break-inside: avoid" in pair_block, "parallel-pair must declare break-inside: avoid for Kindle-safe pagination")
  check.require("(max-width: 32em)" in literary_css, "literary.css must keep the 32em fallback breakpoint for Kindle large font sizes")
  ...
  if classical_width is not None:
    # Keep an upper buffer below 50% to reduce KF8/KFX pixel-rounding wrap risks.
    check.require(
